...
> **Done:** the static parser is gone. `migration-parser.ts` (~870 lines of
> brace-scanning and `new Function()` evaluation) and the `engine` option
> (`migrations.engine`, `MIGRATION_ENGINE`, `--engine`) were deleted;
> execution is now the only way migrations are read.
> `extractTimestampFromFilename` moved to `engine/migration-plan.ts`, which is
> the only caller, and `findMigrationsAfterSnapshot` went with the static path
> — the replayer plans its own file list. `convertPocketBaseMigration`, the
> last text scanner (regex-matching `const snapshot = [...]` out of a file and
> `new Function`-evaluating it), went too: `loadSnapshotIfExists` and
> `loadBaseMigration` execute the snapshot file instead.
> `convertPocketBaseCollection`/`rawCollectionsToSnapshot` stay — they convert
> already-materialized objects, which is what the engine hands them. Tests
> that used `parseMigrationOperations` as an assertion helper now execute
> migrations through `__tests__/helpers/migration-executor.ts`, which reports
> the state a migration produces plus a before/after store diff.

## 1. quickjs-emscripten isolation upgrade
